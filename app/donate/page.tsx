...
export default function DonatePage() {
  ...
  const [addingDonationToBlockchain, setAddingDonationToBlockchain] = useState(false)

  const addDonatedItems = async (
    donorName: string,
    ngoName: string,
    donatedItemName: string,
    donatingDate: number,
    donatedItemQuantity: number
  ) => {
    await connectWallet();
    setAddingDonationToBlockchain(true)
    let donatedItem = {
      donorName: donorName,
      ngoName: ngoName,
      donatedItemName: donatedItemName,
      donatingDate: donatingDate,
      donatedItemQuantity: donatedItemQuantity,
    }
      ...
      if (ethereum) {
        ...
        contract
          .addDonation(
            ...
            donatedItem.ngoName,
            donatedItem.donatedItemName,
            donatedItem.donatingDate,
            donatedItem.donatedItemQuantity
          )
          .then((res: any) => {
            console.log(res)
            setAddingDonationToBlockchain(false)
  ...
  const { ngoName, donorName, units, product, date } =
    useStore(donatedItemsStore)
  const [donationState, setDonationState] = useState<string>("not-started")
  console.log({ ngoName, donorName, units, product, date })
  const router = useRouter()
  const ngoData = [
    {
      name: "Smile Foundation",
      address: "Dwarka Mor, Delhi",
      logo: "https://www.smilefoundationindia.org/wp-content/uploads/2022/09/SMILE-FOUNDATION-LOGO-e1662456150120-1.png",
    },
    {
      name: "Goonj",
      address: "Saket, Delhi",
    ...
    },
    {
      name: "Give India",
      address: "Noida, UP",
      logo: "https://cdn.givind.org/static/images/giveindia-logo-v2.jpg",
    },
    {
      name: "Cry",
      address: "Gurgaon, Haryana",
      logo: "https://upload.wikimedia.org/wikipedia/commons/a/a1/Child_Rights_and_You_%28CRY%29_Organization_logo.png",
    },
  ]
  return (
    <div>
      <div>
        <section className="container items-center justify-center gap-6 pb-8 pt-6 md:py-10">
          <div
            className={
              "grid grid-cols-1 items-center justify-between md:grid-cols-1"
            }
          >
            <div className="mb-6 flex max-w-[980px] flex-col items-start gap-2">
              ...
              {donationState === "not-started" && (
                <p className="max-w-[700px] text-lg text-muted-foreground sm:text-xl">
                  Prakhar, here are the NGOs who are accepting donations within
                  your region.
                </p>
              )}
              {donationState === "selected" && (
                <p className="max-w-[700px] text-lg text-muted-foreground sm:text-xl">
                  Your donation has been finalized ✅! Please proceed to push it
                  to the blockchain.
                </p>
              )}
              {donationState === "done" && (
                <p className="max-w-[700px] text-lg text-muted-foreground sm:text-xl">
                  You are all done, and your delivery has been scheduled. Thank
            ...
            {donationState === "not-started" && (
              <div>
                <div
                  className={
                    "my-3 justify-self-end rounded-md border border-indigo-600 bg-indigo-800 bg-opacity-50 p-3 text-center text-indigo-400 lg:w-fit"
                  }
                >
                  <p>
                    Donating-{" "}
            ...
            <div className={"grid grid-cols-2 gap-4"}>
              {ngoData.map((ngo) => {
                return (
                  <div
                    onClick={() => {
                      setDonationState("selected")
                      donatedItemsStore.setKey("ngoName", ngo.name)
                      donatedItemsStore.setKey("ngoAddress", ngo.address)
                    }}
                    className="flex cursor-pointer items-center rounded-xl border bg-white shadow-sm hover:bg-gray-200 dark:border-gray-700 dark:bg-gray-900 dark:shadow-slate-700/[.7] dark:hover:bg-gray-800"
                  >
                    <div
                      className={
                        "ml-5 flex h-16 w-16 items-center justify-center rounded-full bg-white"
                      }
                    >
                      <img
                        className="w-28 rounded-b-xl object-cover p-2"
                        src={ngo.logo}
                ...
                <div className={"flex items-center justify-center space-x-4"}>
                  <Card>
                    <CardHeader>
                      ...
                      <CardDescription>
                        NSUT, Dwarka Mor, New Delhi
                      </CardDescription>
                    </CardHeader>
                  </Card>
                  <hr className="my-8 h-px w-10 border-0 bg-gray-200 dark:bg-gray-700" />
                  <Card>
                    <CardHeader>
                      <CardTitle>Delivery</CardTitle>
                      <CardDescription>
                        {donatedItemsStore.get().ngoName}{" "}
                        {donatedItemsStore.get().ngoAddress}
                      </CardDescription>
                    </CardHeader>
                  </Card>
                </div>
                ...
                <button
                  ...
                  onClick={() => {
                    addDonatedItems(
                      "Areeb ur Rub",
                      donatedItemsStore.get().ngoName,
                      donatedItemsStore.get().product,
                      new Date().getTime(),
                      donatedItemsStore.get().units
                    ).then((res) => {
                      console.log(res + "Add Donated Items")
                    })
                  }}
                  className="mr-2 mt-5 inline-flex items-center rounded-lg border border-gray-200 bg-white px-5 py-2.5 text-center text-sm font-medium text-gray-900 hover:bg-gray-100 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-700 dark:bg-gray-900 dark:text-white dark:hover:bg-gray-700 dark:focus:ring-gray-600"
                >
                  <svg
                    aria-hidden="true"
              ...
              <div className="mb-6 flex max-w-[980px] flex-col items-start gap-2">
                <div className={"flex items-center justify-center space-x-4"}>
                  <div className={"flex items-center justify-center"}>
                    <Card>
                      <CardHeader>
                        <CardTitle>Scheduled Pickup</CardTitle>
                        <CardDescription>
                          02:00 PM, 25th May, 2023
                        </CardDescription>
                      </CardHeader>
                      <CardContent>
                        Please keep the item ready for pickup at the scheduled
                        time.
                      </CardContent>
                      <CardFooter>
                        Assigned to: &nbsp;
                        <span className="font-bold">Dunzo Pickups</span>
                      </CardFooter>
                    </Card>
                  </div>
                </div>
